...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        val = self.values[0]
        if val == "custom":
            await interaction.response.send_modal(CapsuleModal(self.flow, mode="date"))
        else:
            unit, n = DURATION_OPTIONS[int(val)][1]
            await interaction.response.send_modal(
                CapsuleModal(self.flow, mode="duration", unit=unit, n=n))
...
class TypeView(View):
    ...
    async def _go_time(self, interaction, target):
        try:
            ...
            traceback.print_exc()
            try:
                await interaction.response.send_message(f"صار خطأ 🤔: {e}", ephemeral=True)
            except Exception:
                pass
